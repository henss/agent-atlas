...
import { generateMarkdownViews, renderPackageReadmes, renderRepositoryReadme } from '@agent-atlas/markdown';
...
async function checkGeneratedArtifacts(
  ...
  rootFiles: Array<{ path: string; content: string; overwriteExisting?: boolean }> = [],
): Promise<GeneratedArtifactCheck> {
  const generated = await checkGeneratedMarkdownOutput(outputPath, profile, files);
  const checkedRootFiles = [
    ...(readmePath ? [{ path: readmePath, content: readmeContent, overwriteExisting: true }] : []),
    ...rootFiles,
  ];
  for (const file of checkedRootFiles) {
    const relativePath = normalizeGeneratedPath(file.path);
    const absolutePath = path.resolve(rootPath, file.path);
    try {
      const existing = await readFile(absolutePath, 'utf8');
      if (!file.overwriteExisting && !isGeneratedMarkdownContent(existing)) {
        continue;
      }
      if (existing.replace(/\r\n/g, '\n') !== file.content.replace(/\r\n/g, '\n')) {
        generated.staleFiles.push(relativePath);
      }
    } catch {
      generated.missingFiles.push(relativePath);
    }
  }
...
function isGeneratedMarkdownContent(content: string): boolean {
  return content.includes('<!-- Generated by Agent Atlas. Do not edit directly. -->');
}
...
async function runMaintainCheck(
  ...
  const generated = await checkGeneratedArtifacts(
    ...
    renderConfiguredPackageReadmes(graph, effectivePolicy),
  );
...
async function writeGeneratedMarkdownOutput(
  ...
  profile: AtlasProfile,
  readmePath?: string,
  packageReadmePolicy?: AtlasMaintenancePolicy['generated_package_readmes'],
): Promise<string[]> {
  const graph = await loadAtlasGraph(rootPath, { profile });
  const files = generateMarkdownViews(graph, { profile });
  const packageReadmes =
    packageReadmePolicy?.enabled && packageReadmePolicy.auto_regenerate
      ? renderPackageReadmes(graph, { profile }).map((file) => ({
          ...file,
          overwriteExisting: packageReadmePolicy.overwrite_existing,
        }))
      : [];
  ...
  for (const file of packageReadmes) {
    ...
    if (!file.overwriteExisting) {
      try {
        const existing = await readFile(absoluteFilePath, 'utf8');
        if (!isGeneratedMarkdownContent(existing)) {
          continue;
        }
      } catch {
    ...
    await mkdir(path.dirname(absoluteFilePath), { recursive: true });
    await writeFile(absoluteFilePath, file.content, 'utf8');
    writtenFiles.push(normalizeGeneratedPath(file.path));
  }
  return writtenFiles.sort();
}

function renderConfiguredPackageReadmes(
  graph: AtlasGraph,
  policy: AtlasMaintenancePolicy,
): Array<{ path: string; content: string; overwriteExisting?: boolean }> {
  const packageReadmes = policy.generated_package_readmes;
  if (!packageReadmes?.enabled || !packageReadmes.auto_regenerate) {
    return [];
  }
  return renderPackageReadmes(graph, { profile: policy.profile }).map((file) => ({
    ...file,
    overwriteExisting: packageReadmes.overwrite_existing,
  }));
}
...
switch (command) {
  ...
  case 'generate': {
    ...
    const effectivePolicy = { ...policy, profile: options.profile };
    const readmePath =
    ...
    const packageReadmes = renderConfiguredPackageReadmes(graph, effectivePolicy);
    const writtenRootFiles: string[] = [];
    for (const file of packageReadmes) {
      const absoluteFilePath = path.resolve(options.rootPath, file.path);
      if (!file.overwriteExisting) {
        try {
          const existing = await readFile(absoluteFilePath, 'utf8');
          if (!isGeneratedMarkdownContent(existing)) {
      ...
      await mkdir(path.dirname(absoluteFilePath), { recursive: true });
      await writeFile(absoluteFilePath, file.content, 'utf8');
      writtenRootFiles.push(normalizeGeneratedPath(file.path));
    }
  ...
  case 'diff': {
    ...
    const generated = await checkGeneratedArtifacts(
      ...
      readmePath,
      renderRepositoryReadme(graph, { profile: options.profile }),
      renderConfiguredPackageReadmes(graph, { ...policy, profile: options.profile }),
    );
    const cliReference = await checkConfiguredCliReference(options.rootPath);
    const sourceReference = await checkConfiguredSourceReference(options.rootPath);
    generated.staleFiles.push(...cliReference.staleFiles);
        ...
        policy.generated_docs.auto_regenerate
          ? await writeGeneratedMarkdownOutput(
              ...
              policy.generated_readme?.auto_regenerate
                ? policy.generated_readme.path
                : undefined,
              policy.generated_package_readmes,
            )
          : []
        ),
