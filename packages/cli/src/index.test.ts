...
`);
  ...
  it('suggests cards, checks generated docs, and reports atlas maintenance', async () => {
    ...
    await mkdir(path.join(root, 'docs', 'agents', 'capabilities'), { recursive: true });
    await mkdir(path.join(root, 'docs', 'agents', 'interfaces'), { recursive: true });
    await writeFile(
      path.join(root, 'docs', 'agents', 'capabilities', 'old-skill.md'),
      '<!-- Generated by Agent Atlas. Do not edit directly. -->\n\n# Old Skill\n',
    );
    await writeFile(
      path.join(root, 'docs', 'agents', 'interfaces', 'old-command.md'),
      '<!-- Generated by Agent Atlas. Do not edit directly. -->\n\n# Old Command\n',
    );
    ...
    await expect(
      ...
      stdout: expect.stringContaining('Extra files: 3'),
    });
    ...
    const refreshedCheck = await execFileAsync('node', [
      CLI_PATH,
      'generate',
      'markdown',
      '--path',
    ...
    expect(refreshedCheck.stdout).toContain('Status: passed');
  });
