...
Keep rollout adoption limited to implemented commands. Downstream repos may use `validate`, `show`, `neighbors`, `resolve-path`, `generate markdown`, `context-pack`, `usage-note`, `evaluate`, and read-only MCP resources. Company registry repos may use `atlas global ...`.
...
- Keep all workspace package versions aligned; M12 establishes `0.12.0` as the current local contract version.
