...
The M0-M11 implementation now includes the core schema, validation, graph traversal, path resolution, generated Markdown, context packs, overlays, read-only MCP resources and tools, adapter interfaces, cross-repo registries, migrations, diagnostics, benchmarks, consumer-friendly CLI path handling, version discipline, setup doctor checks, and downstream script templates.
...
Current roadmap work is about making adoption safer and more measurable: usage evidence, boundary checks, incremental authoring, registry hardening, and MCP operational hardening.
