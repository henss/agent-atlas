...
The M0-M14 implementation now includes the core schema, validation, graph traversal, path resolution, generated Markdown, context packs, overlays, read-only MCP resources and tools, adapter interfaces, cross-repo registries, migrations, diagnostics, benchmarks, consumer-friendly CLI path handling, version discipline, setup doctor checks, downstream script templates, local usage receipts, context-pack evaluation, profile boundary checks, and incremental authoring tools.
...
Current roadmap work is about making registry and MCP operations more reliable.
